...
# Cant sort a list of dictionaries without mentioning on which parameter/key the sorting should work
# print(sorted(medals_table))
# Dictionaries can be sorted using sorted() function, but you have to create another function that returns the key
# to sort on.

def sort_key(d: dict) -> str:
    return d['country']

print(f"Dictionary sorted on the basis of country names\n{sorted(medals_table, key=sort_key)}")

# columns = ['country', 'gold', 'silver', 'bronze', 'rank']
columns = ['country', 'gold', 'silver', 'bronze']

out_filename = '../files/OlympicMedals_2020_dict.csv'

with open(out_filename, 'w', encoding='utf-8', newline='') as output_file:
    writer = csv.DictWriter(output_file, fieldnames=columns, extrasaction='ignore')
    # If you don't call writeHeader(), then you will not get headings in the CSV file
    writer.writeheader()
    # 2 ways to use the DictWriter to write the data, you can iterate over the sequence of dictionaries and write the
    # data row by row or send the entire sequence to the writer.
    # Approach 1 ------------>
...
# If you want to exclude some keys if we don't want to include them in the CSV file. Delete rank from columns.
# If we run the file, then we get  raise ValueError("dict contains fields not in fieldnames: "
# ValueError: dict contains fields not in fieldnames: 'rank'. Default behaviour is to raise an error, but we can tell
# DictWriter to ignore the error by passing extrasaction argument in the function as 'ignore'.
# So, now rank column has been excluded from the csv file.

# If some of the dictionaries are missing a key, need to use restval parameter of DictWriter.
